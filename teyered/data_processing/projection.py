...
def project_points(projection_points, r_vectors_all, t_vectors_all,
    ...
    """
    if t_vectors_all.shape[0] != r_vectors_all.shape[0]:
        raise ValueError(
            't_vectors_all and r_vectors_all arrays length must be the same')
...
def calculate_reprojection_error(ground_points_all, projected_points_all):
    ...
    frame
    """
    if projected_points_all.shape[0] != ground_points_all.shape[0]:
        raise ValueError(
            'projected_points_all and ground_points_all array length must be '
            'the same')
    ...
    for i in range(0, ground_points_all.shape[0]):
        if ground_points_all[i] is None:
            errors.append(-1)  # nothing to project
        else:
            errors.append(
                np.linalg.norm(ground_points_all[i] - projected_points_all[i],
                               axis=1).sum()
            )
